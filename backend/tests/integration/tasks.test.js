...
describe('Tasks API Integration Tests', () => {
  ...
  test('POST /api/tasks with valid title returns 201', async () => {
    const res = await request(app).post('/api/tasks').send({ title: 'New Integration Task' });
  ...
  test('PATCH /api/tasks/:id updates status', async () => {
    ...
    const createRes = await request(app).post('/api/tasks').send({ title: 'Task for update' });
  ...
  test('DELETE /api/tasks/:id removes task', async () => {
    const createRes = await request(app).post('/api/tasks').send({ title: 'Task for delete' });
    const id = createRes.body.data.id;
